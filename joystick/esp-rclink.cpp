...
#include "esp-rclink.h"
  ...
  void ESPRCLinkClass::setMode(uint8_t pin, uint8_t type){
	...
	void ESPRCLinkClass::init(String mac, uint8_t mode, uint8_t channel){
		WiFi.mode(WIFI_AP);
		WiFi.softAP(WiFi.softAPmacAddress().c_str(), 0, channel, 1);
		//WiFi.disconnect();
		esp_now_init();
		esp_now_set_self_role(ESP_NOW_ROLE_COMBO);
		esp_now_register_send_cb(&ESPRCLinkClass::send_data);
		esp_now_register_recv_cb(&ESPRCLinkClass::receive_data);
    ...
    ESPRCLinkClass::uploaderTicker.attach_ms(20, &ESPRCLinkClass::uploader);
	}
...
//      Serial.println("===DATA-E===");
	...
	void ESPRCLinkClass::receive_data(uint8_t *mac, uint8_t *data, uint8_t len) {
...
//    Serial.println(len);
	...
	void ESPRCLinkClass::send_data(uint8_t *mac, uint8_t status) {
